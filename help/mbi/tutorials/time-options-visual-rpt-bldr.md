...
但是，對於某些分析，您可能需要在相同的報表中考慮不同的時間範圍或時間間隔。 那就是 `Time` 有選擇。 讓你更清楚地知道如何使用 `Time` 本教學課程涵蓋下列使用案例，供您在報表中使用：
...
有些量度無法隨著時間呈現趨勢，因為系統不會收集或儲存資料以及相關聯的時間戳記。 例如，庫存表格通常每個SKU只包含一列。 在這種情況下，您應 [建立量度](../data-user/reports/ess-manage-data-metrics.md) 而不指定時間戳記。
...
`Time` 選項可讓您建立以時間為基礎的100%圖表，以識別在特定時間範圍內，哪個日、周、月或年貢獻的值最多。 在此區段中，您會建立圖表，顯示每年每個日曆月產生的收入百分比。

如果您想要比較年與年產生的收入，此類型的報表會很實用。 例如，您有一張2015年的圖表顯示，1月份貢獻了該年收入的18%，而2016年的圖表只顯示了8%。 你可以開始研究可能發生的事。
...
1. 接下來，您應用獨立 `Time Interval` 至第一個量度。 按一下 **[!UICONTROL Time Options]** （時鐘圖示） `first Revenue metric`.
...
1. 在下拉式清單中，設定下列項目：
   ...
   * 按一下 **[!UICONTROL Apply]** 以儲存間隔和範圍設定。 這會建立一個量度來計算前一年的總收入。 接下來，您將此量度當做公式中的分母。

   * 若要查看每個月的收入百分比，您必須新增公式至報表。 按一下 **[!UICONTROL Add Formula]**.
...
若要展示此使用案例，請查看上個月的每日收入與上年同期相比。 假設您想查看2016年1月的每日收入，然後比較2015年1月、2014年1月等 — 此報表會顯示這些資訊。
...
1. 在下拉式清單中，設定下列項目：
   ...
   * `Time Range`:將此項設定為 `From 14 Months Ago To 13 Months Ago` 按一下 **[!UICONTROL Custom]** then **[!UICONTROL Moving Range]**. 使用功能表頂端的欄位和下拉式清單來設定範圍。 此設定可讓我們查看前一個月（但前一年）的收入。
   不要擔心量度會從報表中消失 — 設定獨立時間選項會自動從報表中隱藏量度。 若要重新顯示，請按一下 **[!UICONTROL Show]** 在量度旁。

   ![](../assets/Different_Time_Ranges.gif)

   * 按一下 **[!UICONTROL Apply]** 以儲存間隔和範圍設定。

   * 接下來，您新增自訂 `Day number of the month` 維度，按一下 **[!UICONTROL Group By]** 和選取維度。 這會傳回訂單月份的天數，例如，在3月2日下的訂單會傳回 `2`.

   * 在 `Group By` 下拉式清單，選取 `Show All` 按一下 **[!UICONTROL Apply]**. 這會建立報表的X軸值：
...
1. 新增 `Items Sold` 量度。
1. 按一下 **[!UICONTROL Duplicate]** 來製作量度的復本。
1. 重新命名量度。 您可以使用相同的名稱或使用類似的名稱：
   1. 將第一個量度重新命名為 `Items sold last 7 days`.
   1. 將第二個量度重新命名為 `Items sold last 28 days`.
1. 在 `Items sold last 7 days` 量度，按一下全域 **[!UICONTROL Time Range]** 選項，然後 **[!UICONTROL Moving Time Range]**. 在此範例中，您會將其設為 `Last 7 Days`.
1. 按一下 **[!UICONTROL Time Interval]** 並將其設定為 `None`.
...
1. 在下拉式清單中，設定下列項目：
   ...
   * 複製 `Items sold last 28 days` 量度並開啟新量度的 `Time Options`. 將選項設為：
      ...
      * 新增 `Units on hand` 量度。
      * 接下來，您必須新增計算，根據銷售趨勢，顯示時段(`last 7 days`, `last 28 days`，和 `last promo` 期間)，您會納入報表。 您必須對每個時段執行一次此動作。
...
* 若 `last 28 days time period`，輸入 `D / (B/4)` 在 `Formula` 欄位。
   ...
   >[!NOTE]
   >
   >請務必在此將您選取的時間範圍正規化。 在此範例中，將28天分為4週。 您可能需要對公式套用不同的邏輯。
...
此範例說明目前的庫存水準在整個產品14天的銷售中皆已妥善設定。 但是，增加一個可比的促銷期意味著公司需要做出一些改變 — 要麼訂購更多庫存，要麼只推銷庫存量足夠的商品。
