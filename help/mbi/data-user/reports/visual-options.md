...
若要將報表儲存為標量，請配置篩選器和時間設定，然後按一下 **[!UICONTROL Save]** 或 **[!UICONTROL Update]** 的上方。 在 `Type` 下拉式清單，選擇數字：量度名稱，將報表儲存為左側列顯示的值。
...
正如名字所暗示的， `table` 報表非常適合用於顯示表格詳細資訊。 如果需要在單一報表中依值或量度顯示許多群組，表格通常是最佳方式。 以下是「客戶詳細資料」表格，顯示依客戶電子郵件分組的訂單和收入：
...
每個不重複的量度、分組依據和時間間隔組合都會顯示為其專屬列。 如果您有兩個量度，其中一個 `group by`，包含三個不同 `group by` 值，則報表會顯示6個不同的長條。
...
例如，以下報表有兩個相同的收入量度，一個篩選了第一次訂購，另一個篩選了重複訂購。 依商店分組後，您會看到每個商店的總收入貢獻（以橫條的總寬度表示），以及每個商店的首次與重複收入劃分。
...
`Column` 圖表將每個資料點表示為垂直欄，與水準長條圖視覺效果相比，圖表更適合顯示時間趨勢資料。 每個不重複量度和組合在其自己的一系列長條中表示。 欄報表最適合含有3個或更少量度的報表，或包含1-3個依值分組且具有單一群組的一個量度。

在以下範例中，您會看到兩個收入量度，一個是篩選為首次收入，另一個是篩選為重複收入，並隨月呈現趨勢：
...
在漏斗報表中，漏斗的指定步驟的相對值會反映在該步驟的高度。 報表設定會決定步驟的顯示順序。 設定漏斗報表有兩種方式：
...
A `bubble` 圖表最多可顯示四個資料維度，其中 `X` 和 `Y` 軸指定氣泡的位置。 此 `Z` 軸是泡泡的大小，通過包括兩個組，可以向泡泡添加顏色。 當您想要在單一圖表中繪製多個資料維度時，最適合使用此類型的視覺效果。
