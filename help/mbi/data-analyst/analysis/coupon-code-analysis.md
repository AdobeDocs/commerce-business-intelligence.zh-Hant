...
首先，您必須確保將下列欄同步至Data Warehouse。 如果沒有，請瀏覽至「`Manage Data` > `Data Warehouse`」並同步處理下列專案，繼續追蹤他們：
...
* `sales\_flat\_order`資料表
   ...
   * **分析人員建立的客戶第一筆訂單優惠券**{：}**-**，作為\[優惠券分析\]票證的一部分

* **分析人員建立作為\[COUPON ANALYSIS\]票證一部分的**{：}**-**&#x200B;使用之客戶期限優惠券數目
* **優惠券贏取客戶或非優惠券贏取客戶**
   * [!UICONTROL Column type]： `Same Table => CALCULATION`
   * [!UICONTROL Inputs]：
      * `A`： `Customer's first order included a coupon? (Coupon/No coupon)`
