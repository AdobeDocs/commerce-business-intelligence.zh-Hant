...
MBI讓您輕鬆 [結合廣告成本資料和收入資料](../../data-analyst/importing-data/integrations/google-adwords.md) 從資料庫。 這可協助您識別哪些促銷活動的投資報酬率最高。 本文探討評估行銷活動績效的幾種不同方法。

## 必要條件

* 匯入廣告成本資料：
   * [連接您的 [!DNL Google AdWords] to [!DNL MBI]](../importing-data/integrations/google-adwords.md):這會同步您的 [!DNL Adwords] 花在 [!DNL MBI]
   * [上傳其他廣告成本資料](../importing-data/connecting-data/import-offline-ad-data.md):若沒有直接連接器，建議使用 [!DNL MBI]
   * 如果您從多個來源匯入成本資料，您可以 [合併](../../best-practices/consolidating-your-tables.md) 中的資料 [!DNL MBI]. 簡單 [提交支援票證](../../guide-overview.md).
...
以上分析(1)和(2)在關於 [識別最上層行銷管道](../analysis/most-value-source-channel.md). 在此，您將探索分析(3)，以衡量一段時間中的促銷活動ROI。 這可回答從特定促銷活動取得的使用者是否產生足夠的終身收入來支付購買成本。

>[!NOTE]
>
>此範例假設所有促銷活動成本都專門用於贏取新使用者。 實際上，您的促銷活動成本也會與取得未轉換的造訪、重複購買者等項目共用。 假設所有成本都用於獲取新註冊用戶，則最終的ROI將計入最壞情況（每次獲取的最高成本）。 您可以確定您的實際ROI高於您的計算。
>
>範例：假設您在產生10個新使用者和10個重複購買者的促銷活動上花費$20，則每個新使用者的實際成本為$1。 但是，假定所有成本都是為了收購新用戶，每次收購的成本是2美元。)
...
1. 按一下 `Back to Metric List` ，然後前往任何控制面板。
...
1. 此報表會依促銷活動顯示您的平均使用者期限收入

**最後，將這三項分析整合在一份報表中，借此計算促銷活動的投資報酬率：**

1. 在任何控制面板中，按一下 **[!UICONTROL Add Report > Create new report]**
1. 新增作為輸入，請使用上述三個量度。 每個都會獲派一個字母(例如，\[`A`\], \[`B`\]和\[`C`\]
1. [!UICONTROL Cost]:新增量度AdWords成本 — 此為變數\[A\]。 這會依促銷活動傳回成本。
1. [!UICONTROL Users]:新增量度「新使用者」 — 此為變數\[B\]。 這會依促銷活動傳回使用者人數。
1. [!UICONTROL LTV]:新增「平均期限收入」量度 — 此為變數\[`C`\]。 這會依行銷活動傳回LTV。
...
1. [!UICONTROL Avg Return per User]:輸入公式 **\[`C`\]-(\[`A`\]/\[`B`\]**. 這會傳回透過計算（平均使用者LTV） — （每次贏取的平均成本）而對使用者產生的平均利潤。
...
您在經營產品專屬廣告嗎？ 若是如此，您可以透過計算特定產品的收入/成本來評估這些促銷活動的投資報酬率。

>[!NOTE]
>
>此範例假設所有促銷活動成本都專門用於產生特定產品的購買。 假設所有成本都用在了產生購買上，所得的ROI就會計入最壞的情況（每次購買的最高成本）。 您可以確定您的實際ROI高於此計算。 範例：假設您在產生10位新使用者和10次購買的促銷活動上花費$20，則您的每次購買實際成本為$1。 假設所有購買新用戶的成本都為2美元。)*
...
**2. 若要計算ROI，請再次將量度合併至一個報表：**

1. 在任何控制面板中，按一下 **[!UICONTROL Add Report > Create new report]**
1. 新增 `Revenue by items` 量度，依據上方特定產品報表之促銷活動的指示進行篩選並分組，然後按一下 **[!UICONTROL Hide]** 在量度的標量值下
1. 現在新增 [!DNL AdWords Cost] 量度，依篩選器，並依來自 `Ad cost by campaigns` 您在 `User acquisition campaigns` 上節；然後按一下 **[!UICONTROL Hide]** 在量度的標量值下
1. 備妥這些量度後，新增公式：
...
1. [!UICONTROL Return]:輸入公式 `\[A\]-\[B\]`. 這會透過計算（平均使用者LTV） — （每次贏取的平均成本），傳回對使用者的平均利潤
