...
此 `sales_order` 表格(`sales_flat_order` 在M1)，是擷取每筆訂單的位置。 在大多數情況下，每一列代表一個唯一順序，不過有些自訂商務實作會將順序分割為個別列。

此表包含所有客戶訂單，無論該訂單是否通過來賓結帳處理。 如果您的商店接受來賓結帳，您可以找到有關此的更多資訊 [使用案例](../data-warehouse-mgr/guest-orders.md).
...
| `base_subtotal` | 訂單中所有項目的總商品價值。 稅、運費、折扣等不包括在內 |
...
| `coupon_code` | 套用至訂單的抵用券。 如果未套用抵用券，此欄位將會 `NULL` |
| `created_at` | 訂單的建立時間戳記，通常以UTC儲存在本機。 視您在 [!DNL MBI]，此時間戳記可轉換為 [!DNL MBI] 與資料庫時區不同 |
...
| `customer_id` | `Foreign key` 與 `customer_entity` 表中，如果已註冊客戶。 加入 `customer_entity.entity_id` 確定與訂單關聯的客戶屬性。 如果訂單是透過訪客結帳下達，此欄位將會是 `NULL` |
| `entity_id` (PK) | 表的唯一標識符，常用於連接到Commerce實例內的其他表 |
| `increment_id` | 訂單的唯一識別碼，通常稱為 `order_id` Magento。 此 `increment_id` 最常用於連接到外部源，例如 [!DNL Google Ecommerce] |
...
| `Customer's order number` | 此客戶訂單的循序排名。 識別客戶下的所有訂單，依 `created_at` 時間戳記，並為每個順序指派遞增的整數值。 例如，客戶的首筆訂單會傳回 `Customer's order number` 1;其二階傳回 `Customer's order number` 2;等等 |
...
| `Is customer's last order?` | 確定訂單是否與客戶的最後訂單或最近訂單相對應。 透過比較 `Customer's order number` 值 `Customer's lifetime number of orders`. 當這兩個欄位在指定順序中相等時，此欄會傳回「是」；否則會傳回「否」 |
...
| `Seconds since previous order` | 此訂單與客戶之前訂單之間的間隔時間。 減去 `created_at` 從 `created_at` ，以整數秒數傳回。 例如，對於與客戶第三筆訂單相對應的訂單記錄，此欄會傳回客戶第二筆訂單與第三筆訂單之間的秒數。 對於客戶的首筆訂單，此欄位將返回 `NULL` |
...
| `Revenue` | 所有訂單的收入總和，其中收入定義為客戶支付的最終價格，並應用所有稅、折扣、發運等 | `Operation: Sum`<br/>`Operand: base_grand_total`<br/>`Timestamp:` `created_at` |
...
| `Unique Customers` | 在指定報告時間間隔內下訂單的不重複客戶數。 例如，如果報表的間隔是每週，則在指定周內至少下一筆訂單的每位客戶，無論在該周內下了多少訂單，都將被計算一次 | `Operation: Count Distinct`<br/>`Operand:` `customer_email`<br/>`Timestamp:` `created_at` |
...
* 加入 `customer_group` 表格，以建立新欄，以傳回下訂單之客戶的客戶群組名稱。
   * 路徑： `sales_order.customer_group_id` （多個）=> `customer_group.customer_group_id` (1)

`sales_order_address`

* 加入 `sales_order_address` 表，用於建立與訂單關聯的新列，以返回開單和發運地點。 根據需要計費還是發運詳細資訊，可以使用兩個連接路徑。
   * 路徑：
...
* 加入 `store` 表格，以建立新欄，這些欄會傳回與訂單相關聯的商務存放區詳細資訊。
