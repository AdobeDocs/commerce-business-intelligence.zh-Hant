---
title: sales_order表格
description: 探索Commerce Intelligence Data Warehouse中的sales_order表格結構。 瞭解每一列如何代表順序，以及自訂分割的發生位置。
...
| **資料行名稱** | **描述** |
|---|---|
| `Billing address city` | 訂單的帳單城市。 透過加入`sales_order`.`billing_address_id`計算 至`sales_order_address`.`entity_id` 並傳回`city`欄位 |
| `Billing address country` | 訂單的帳單國家/地區代碼。 透過加入`sales_order`.`billing_address_id`計算 至`sales_order_address`.`entity_id` 並傳回`country_id` |
| `Billing address region` | 訂單的帳單區域（通常為州或省）。 透過加入`sales_order`.`billing_address_id`計算 至`sales_order_address`.`entity_id` 並傳回`region`欄位 |
| `Customer's first order date` | 此客戶下第一個訂單的時間戳記。 通常被認為是客戶的「贏取日期」。 傳回最小值`sales_order`.`created_at`計算 每個獨特客戶的值 |
...
| `Customer's group code` | 下此訂單的客戶的群組名稱。 透過加入`sales_order`.`customer_group_id`計算 至`customer_group`.`customer_group_id` 並傳回`customer_group_code`欄位 |
...
| `Number of items in order` | 訂單中包含的料號總數。 透過加入`sales_order`.`entity_id`計算 至`sales_order_item`.`order_id` 並加總`sales_order_item`.`qty_ordered` 欄位 |
...
| `Shipping address city` | 訂單的送貨城市。 透過加入`sales_order`.`shipping_address_id`計算 至`sales_order_address`.`entity_id` 並傳回`city`欄位 |
| `Shipping address country` | 訂單的出貨國家/地區代碼。 透過加入`sales_order`.`Shipping_address_id`計算 至`sales_order_address`.`entity_id` 並傳回`country_id` |
| `Shipping address region` | 訂單的送貨地區（通常為州或省）。 透過加入`sales_order`.`shipping_address_id`計算 至`sales_order_address`.`entity_id` 並傳回`region`欄位 |
| `Store name` | 與此訂單相關聯的Commerce商店名稱。 透過加入`sales_order`.`store_id`計算 至`store`.`store_id` 並傳回`name`欄位 |
