...
這些類型的欄可由Data Warehouse分析師的Adobe團隊建立。 若要定義新的計算欄，請提供下列資訊：
...
這稱為 **事件數** 計算欄。 這表示您正在嘗試尋找發生特定事件擁有者（例如客戶或使用者）之事件的順序。
...
這稱為 **循序事件比較**. 這表示您正嘗試尋找值（貨幣、數字、時間戳記）與擁有者先前事件之對應值之間的差值。
...
別擔心。 僅僅因為此處未列出，並不表示這是不可能的。 Data Warehouse分析師的Adobe團隊可以提供幫助。
