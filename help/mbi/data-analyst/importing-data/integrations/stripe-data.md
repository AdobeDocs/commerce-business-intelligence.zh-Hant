...
本主題會探索您可從其中匯入的主要資料表 [!DNL Stripe] 到 [!DNL Commerce Intelligence]. 安裝完成後，會在您的Data Warehouse中建立下清單格。 按一下「表格名稱」資料欄中的連結，以進一步瞭解每個表格中的屬性。
...
| [`Events`](https://stripe.com/docs/development/dashboard/events) | 事件可讓您瞭解帳戶中發生的有趣事件。 [當有趣的事件發生時](https://stripe.com/docs/api/events/types)，則會建立新的事件物件。 例如，當充電成功時 `charge.succeeded` 事件已建立；或當無法支付商業發票時 `invoice.payment\_failed` 事件已建立。 |
