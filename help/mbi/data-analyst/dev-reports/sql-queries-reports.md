...
有人想知道SQL查詢如何轉譯為 [計算欄](../data-warehouse-mgr/creating-calculated-columns.md), [量度](../../data-user/reports/ess-manage-data-metrics.md)，和 [報告](../../tutorials/using-visual-report-builder.md) 在 [!DNL MBI]？如果您是大量SQL用戶，請了解SQL在 [!DNL MBI] 會讓您在 [Data Warehouse管理員](../data-warehouse-mgr/tour-dwm.md) 最好地利用 [!DNL MBI] 平台。

在本文結尾，我們已將 **翻譯矩陣** 針對SQL查詢子句和 [!DNL MBI] 元素。
...
此示例涵蓋了大多數翻譯案例，但有一些例外。 讓我們深入，從 `aggregate` 函式。
...
讓我們來看看 `Total Revenue` 量度的定義 [!DNL MBI]. 請查看下面的查詢，我們將嘗試翻譯：
...
按一下「 」，導覽至量度產生器 **[!UICONTROL Manage Data** > **&#x200B;量度&#x200B;**> **建立新量度]**，首先必須選取適當 `source` 表格，在此例中為 `orders` 表格。 接著會設定量度，如下所示：
...
在 [!DNL MBI] 需要使用Data Warehouse管理器，您可在此建立 `orders` 和 `customers` 表格，然後建立名為 `Customer LTV` 在客戶的桌子上。

首先，讓我們看看如何在 `customers` 和 `orders`. 我們的最終目標是在 `customers` 表格，請先導覽至 `customers` 表格，然後按一下 **[!UICONTROL Create a Column** > **&#x200B;選取定義&#x200B;**> **SUM]**.

接下來，您需要選擇源表。 如果路徑已存在，則 `orders` 表格，只需從下拉式清單中選取。 不過，如果您要建立新路徑，請按一下 **[!UICONTROL Create new path]** 螢幕將顯示如下：
...
儲存路徑後，所有人都會設定建立新 `Customer LTV` 欄目！ 請查看下列項目：

![](../../assets/Customer_LTV.gif)

既然你已經建立了 `Customer LTV` 欄 `customers` 表格，您可以建立 [量度匯總](#aggregate) 使用此欄（例如，找出每位客戶的平均LTV），或只是 `group by` 或 `filter` 計算欄，使用 `customers` 表格。

>[!NOTE]
>
>對於後者，每次您建立新的計算欄時，都需要 [將維度新增至現有量度](../data-warehouse-mgr/manage-data-dimensions-metrics.md) 之前，請先以 `filter` 或 `group by`.

請參閱 [建立計算列](../data-warehouse-mgr/creating-calculated-columns.md) 和Data Warehouse管理員。

## `Group By` 條款

`Group By` 查詢中的函式通常表示在 [!DNL MBI] 作為欄，用於劃分或篩選視覺化報表。 例如，請讓我們重新造訪 `Total Revenue` 查詢，但這次讓我們依 `coupon\_code` 以便更清楚了解哪些優惠券能產生最多的收入。
...
>[!NOTE]
>
>與我們之前開始的查詢相較，唯一的差異是新增「抵用券\_code」作為群組依據。_

使用相同 `Total Revenue` 我們先前建立的量度，現在可以建立依抵用券代碼劃分的收入報表了！ 請查看下方的gif圖，其中顯示如何設定此視覺化報表，查看9月至11月的資料：

![按抵用券代碼列出的收入](../../assets/Revenue_by_coupon_code.gif)

## 公式

在某些情況下，查詢可能涉及多個聚合，以計算不同列之間的關係。 例如，您可以透過下列兩種方式之一，計算查詢中的平均訂單值：
...
我們也假設我們已設定量度來計算 `Total Revenue` 和 `Number of orders`. 由於這些量度已存在，因此我們只需開啟 `Report Builder` 並使用 `Formula` 功能：

![AOV公式](../../assets/AOV_forumula.gif)

## 包裝

正如我們在本文開頭提到的，如果您是一個龐大的SQL用戶，請思考查詢如何轉譯 [!DNL MBI] 可讓您建立計算欄、量度和報表。
